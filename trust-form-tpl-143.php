...
//		$html .= '<tr><th scope="row"><div class="subject"><span class="content">'.$name.'</span>'.(isset($validate[$key]['required']) && $validate[$key]['required'] == 'true' ? '<span class="require">'.$config['require'].'</span>' : '' ).'</div><div class="submessage">'.$attention[$key].'</div></th><td><div>'.$trust_form->get_element( $key ).'</div>';
        ...
        $html .= '<td><input class="pl-uppercase" name="'.$key.'" placeholder="'.$name.'">';
...
$html .= <<<EOT
<p id="confirm-button" class="submit-container">{$trust_form->get_form('motivo_input')}</p>
<br/>
</form>
</div>
EOT;
...
$html .= <<<EOT
<p id="confirm-button" class="submit-container">{$trust_form->get_form('motivo_confirm')}</p>
<br/>
</form>
</div>
EOT;
